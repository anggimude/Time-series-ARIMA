...
sales_ts <- ts(sales, frequency = 12)  # monthly data, frequency = 12
...
#| warning: false
#Q2(iii)
# calculae the different arima models
...
tsdisplay(residuals(arima110), main="Residuals of ARIMA(1,1,0)")
```
...
tsdisplay(residuals(arima111), main="Residuals of ARIMA(1,1,1)")
```
...
salt.spec <- spectrum(salt, 
                      ...
                      log = "no")  # 'log="no"' to avoid log-scaling on the y-axis
...
The spectral analysis peaks around the frequency of 0.0625 which corresponds to around 16 time units. The confidence interval of [16.661, 36880.941] is a large because the degree of freedom is low. This indicates high uncertainty of the exact magnitude of the peak but since a peak exists, it suggests that there is a meaningful cycle at 16 time units. 